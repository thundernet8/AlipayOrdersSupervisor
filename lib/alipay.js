...
var pusher = new push_1.default(config_1.default.pushStateAPI, config_1.default.pushAppId, config_1.default.pushAppKey, config_1.default.pushStateSecret);
...
function restoreOrderList() {
    ...
    var ordersString = fs.readFileSync("./orders/" + filename).toString();
    try {
...
function parseOrdersHtml(html) {
    ...
    orderRows.each(function (_index, _ele) {
        ...
        orderData.time =
            trim_1.default(timeSel.first().text()) + " " + trim_1.default(timeSel.last().text());
        // 备注
        orderData.memo = trim_1.default(orderRow.find(".memo-info").text());
        // 订单描述
        orderData.description = trim_1.default(orderRow
            .children("td.name")
            .children("p")
            .text());
        ...
        if (orderNoData.length > 1) {
            orderData.orderId = trim_1.default(orderNoData[0].split(":")[1]);
            orderData.tradeNo = trim_1.default(orderNoData[1].split(":")[1]);
        }
        else {
            orderData.tradeNo = trim_1.default(orderNoData[0].split(":")[1]);
        }
        // 对方支付宝用户名
        orderData.username = trim_1.default(decodeUnic(orderRow
            .children("td.other")
            .children("p")
            .text()));
        // 金额
