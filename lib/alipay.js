...
var spiderWorker_1 = require("./spiderWorker");
...
function restoreOrderList() {
    ...
    var ordersString = fs.readFileSync(path.resolve(__dirname, "../orders/" + filename));
    try {
...
function backupOrderList() {
    ...
    var filename = moment().format("YYYY_MM_DD") + ".json";
    fs.writeFileSync(path.resolve(__dirname, "../orders/" + filename), ordersString);
}
...
function checkOrderListPageHtmlString() {
    ...
    spiderWorker_1.default()
        .then(function (result) {
        timePrefixLog("Fetch orders page content successfully");
        fs.writeFile(path.resolve(__dirname, "../orders.json"), result, function () { });
...
function parseOrdersJSON(results) {
    var list = JSON.parse(results).result.detail;
    list.forEach(function (item) {
        ...
        // 金额
        var amountText = item.tradeConfirmAmount; // + 100.00 / - 100.00 / 100.00
...
function dailyReport() {
    // Email报告
    var now = moment();
    mailer.sendMail("Alipay Supervisor Service Daily Report(" +
